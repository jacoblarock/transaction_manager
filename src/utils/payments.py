...
def create_payment(
    ...
    logger.info(f"create payment user={u_id} group={g_id} recipient={recipient_u_id} amount={p_amount}")
...
def update_payment(
    ...
    u_id = check_auth(s_token)
    logger.info(f"update payment user={u_id} payment={p_id} amount={p_amount}")
    with db.connect() as conn:
        payment_rows = db.select(
            conn,
...
def get_payments(
    ...
    with db.connect() as conn:
        ...
        return db.select(
            conn,
            "select p_id, p_u_sender, p_u_recipient, p_g_ref, p_amount, p_created_at "
